...
if ! command -v rsync &> /dev/null; then
    echo "-> [错误] 未找到 rsync 命令，请先安装: yum install rsync 或 apt install rsync"
    exit 1
fi

echo "[1/5] 创建临时目录..."
mkdir -p "$TEMP_DIR"
echo "[2/5] 正在导出数据库: $DB_NAME ..."
...
for remote_conn in "${!site[@]}"; do
    ...
    rsync -avP -e ssh "$BACKUP_ROOT/$ARCHIVE_NAME" "$remote_conn:$path/"
