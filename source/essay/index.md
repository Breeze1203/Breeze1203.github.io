        ...
        .then(data => {
            ...
            entries.forEach(entry => {
                ...
                if (hasTargetCategory) {
                    ...
                    let dateString = '';  // 改为存储 ISO 格式的日期字符串，用于可靠排序
                    // 1. 使用正则表达式匹配 URL 中的 YYYY/MM/DD 格式
                    const dateMatch = url.match(/(\d{4})\/(\d{2})\/(\d{2})/);
                    if (dateMatch) {
                        // 2. 如果匹配成功，则拼接成 YYYY-MM-DD 格式
                        ...
                        if (dateElement) {
                            dateString = dateElement.textContent.trim();  // 假设是 ISO 格式，如 2025-11-23T...，new Date 会处理
                        }
                    }
                    // =======================================================
                    doodlesData.push({ title, url, date: dateString });
                }
            });
            ...
            // 排序：使用 ISO 日期字符串，确保 new Date 能正确解析，按降序（最新在最前）
            doodlesData.sort((a, b) => {
                const dateA = a.date ? new Date(a.date) : new Date(0);  // 无日期的排到最后
            ...
            doodlesData.forEach(item => {
                ...
                // 显示时再格式化为 zh-CN 格式
                dateSpan.textContent = item.date ? `${new Date(item.date).toLocaleDateString('zh-CN', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric'
                })}-` : '';
